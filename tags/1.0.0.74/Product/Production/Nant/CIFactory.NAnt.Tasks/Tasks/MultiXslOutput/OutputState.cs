namespace Common.Functions.MultiXslOutput
{
    ...
    internal class OutputState {            
        ...
        public void InitWriter() {
            ...
            if (Append)
            {
                writer = new FileStream(Href, FileMode.Append);
            }
            else
            {
                writer = new FileStream(Href, FileMode.Create, FileAccess.Write, FileShare.Read);
            }
            if (method == OutputMethod.Xml) {
                if (!Fragment)
                {
                    xmlWriter = new XmlTextWriter(writer, encoding);
                }
                else
                {
                    xmlWriter = new XmlFragmentWriter(writer, encoding);
                }
                if (indent)
                ...
                textWriter = new StreamWriter(writer, encoding);
