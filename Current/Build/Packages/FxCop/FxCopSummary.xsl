...
<xsl:stylesheet
          ...
          <table
            ...
            <tr>
              <td class = "sectionheader" colSpan = "4">
                FxCop
                <xsl:value-of select = "$fxcop.version"/>
                Summary
              </td>
            </tr>
            <tr>
  ...
  <xsl:template match = "Class">
    <div class = "fxcop-class">
      <table style = "WIDTH: 30pc">
        <tr>
          <td colspan = "2" class = "{$applicationPath}/fxcop-class-header">
  ...
  <xsl:template match = "Field">
    <tr>
      <td width = "20%">
        <img src = "{$applicationPath}/images/field.gif"/>
        <xsl:value-of select = "@Name"/>
      </td>
      <td>
        <xsl:apply-templates select = "Messages">
  ...
  <xsl:template match = "Method" mode = "fromClass">
    <tr>
      <td colspan = "2">
        <img src = "{$applicationPath}/images/method.gif"/>
        <xsl:value-of select = "@Name"/>
      </td>
    </tr>
    <tr>
  ...
  <xsl:template match = "Property">
    <tr>
      <td colspan = "3">
        <img src = "{$applicationPath}/images/property.gif"/>
        <xsl:value-of select = "@Name"/>
      </td>
    </tr>
    ...
    <div class = "section-data" style = "margin-left:10px">
      <img>
        ...
        <xsl:if test = "$level='Warning'">
          <xsl:attribute name = "src">
            <xsl:value-of select = "$applicationPath"/>
            /images/fxcop-warning.gif
          </xsl:attribute>
        </xsl:if>
        <xsl:if test = "$level='CriticalWarning'">
          <xsl:attribute name = "src">
            <xsl:value-of select = "$applicationPath"/>
            /images/fxcop-critical-warning.gif
          </xsl:attribute>
        </xsl:if>
      ...
      <a>
        <xsl:attribute name = "href">
          <xsl:value-of select = "$fxcop.root/Rules/Rule[@TypeName=$ruleName]/Url"/>
        </xsl:attribute>
