  ...
  <xsl:param name="ArtifactUrl" />
    ...
    <IMG>
      <xsl:attribute name="SRC">
        <xsl:value-of select="$ArtifactUrl"/>/AbstracnessVSInstability.png
      </xsl:attribute>
    </IMG>
		<p/>
		<p/>
    ...
    <IMG>
      <xsl:attribute name="SRC">
        <xsl:value-of select="$ArtifactUrl"/>/ComponentDependenciesDiagram.png
      </xsl:attribute>
    </IMG>
		<p/>
		<p/>
    ...
    <xsl:template match="InfoWarnings">
        ...
        <xsl:for-each select="WarningByType"><p/><div class="infobold"><xsl:value-of select="@TypeName"/></div><xsl:apply-templates select="Warning"/></xsl:for-each>
