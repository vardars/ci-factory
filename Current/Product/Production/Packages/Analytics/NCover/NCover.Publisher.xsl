...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    ...
    <integration>
      ...
      <xsl:variable name="BuildDate" select="($CCNetLog)/cruisecontrol/build/@date"/>
      <xsl:if test="BuildDate != ''">
        <xsl:attribute name="day">
          <xsl:value-of select="ms:FormatDate(BuildDate, 'dd')"/>
        </xsl:attribute>
        <xsl:attribute name="month">
          <xsl:value-of select="ms:FormatDate(BuildDate, 'MM')"/>
        </xsl:attribute>
        <xsl:attribute name="year">
          <xsl:value-of select="ms:FormatDate(BuildDate, 'yyyy')"/>
        </xsl:attribute>
        <xsl:attribute name="week">
          <xsl:value-of select="ms:WeekNumber(BuildDate)"/>
        </xsl:attribute>
        <xsl:attribute name="dayofyear">
          <xsl:value-of select="ms:DayOfYear(BuildDate)"/>
        </xsl:attribute>
        <xsl:attribute name="hourofday">
          <xsl:value-of select="ms:FormatDate(BuildDate, '%H')"/>
        </xsl:attribute>
      </xsl:if>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'StartTime'"/>
        <xsl:with-param name="StatisticValue" select="BuildDate"/>
      </xsl:call-template>
      <xsl:call-template name="AddStatistic">
      ...
      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'solutionTotalCoveragePercentage'"/>
        <xsl:with-param name="StatisticValue" select="/coverageReport2[1]/project[1]/@coverage"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'solutionFunctionCoveragePercentage'"/>
        <xsl:with-param name="StatisticValue" select="/coverageReport2[1]/project[1]/@functionCoverage"/>
      </xsl:call-template>
