...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    <xsl:variable name="BC30149" select="count(/cruisecontrol/build/log/builderror/message[contains(text(), 'error BC30149')]) > 0" />
    <xsl:variable name="BC30401" select="count(/cruisecontrol/build/log/builderror/message[contains(text(), 'error BC30401')]) > 0" />
    <integration>
      ...
      <xsl:attribute name="status">
        <xsl:choose>
          ...
          <xsl:when test="$BC30149 and $BC30401">
      ...
      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'trackercount'"/>
        <xsl:with-param name="StatisticValue" select="count(//TrackerQuery/Tracker)"/>
      </xsl:call-template>
