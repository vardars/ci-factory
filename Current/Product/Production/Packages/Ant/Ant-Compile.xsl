            ...
            <td height="42" class="sectionheader-container-error" colSpan="2">
              <a STYLE="TEXT-DECORATION: NONE; color: D13535;" onmouseover="this.style.color = '#403F8D'" onmouseout="this.style.color = '#D13535'">
                <xsl:attribute name="href">
                  https://&HostName;/&ProjectName;-&ProjectCodeLineName;/default.aspx?_action_AntReport=true&amp;server=<xsl:value-of select="$CCNetServer" />&amp;project=<xsl:value-of select="$CCNetProject" />&amp;build=<xsl:value-of select="$CCNetBuild" />
                </xsl:attribute>
                <img src="https://&HostName;/&ProjectName;-&ProjectCodeLineName;/Packages/Ant/logo.gif" class="sectionheader-title-image"/>
